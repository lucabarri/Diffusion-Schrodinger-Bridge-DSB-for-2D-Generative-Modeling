...
Create a Python virtual environment and install the required packages listed in `requirements.txt`:
...
Launch Jupyter and open `ML_stoch_control_proj_BARRIVIERA_DIDASCALOU.ipynb`:

```bash
jupyter notebook
```

On a modern GPU (e.g., an NVIDIA 1080 Ti or better) the full training in the notebook runs in roughly 10–15 minutes. On CPU it can take around an hour.
